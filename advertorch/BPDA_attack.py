...
def get_cw_grad(adv, origin, label, encoder, classifier):
    logits = classifier(encoder(adv))
    ce = nn.CrossEntropyLoss()
    l2 = nn.MSELoss()
...
def BPDA_attack(image, target, encoder, classifier, step_size = 1., iterations = 10, linf=False, transform_func=identity_transform):
    ...
    for _ in range(iterations):
        ...
        loss = l2(0, adv_def)
        loss.backward()
        g = get_cw_grad(adv_def, image, target, encoder, classifier)
        if linf:
            g = torch.sign(g)
        print(g.numpy().sum())
        ...
        adv.requires_grad_()
        if linf:
            print('label', torch.argmax(classifier(encoder(adv))), 'linf', torch.max(torch.abs(adv - image)).detach().numpy())
        else:
            print('label', torch.argmax(classifier(encoder(adv))), 'l2', l2_norm(adv, image))
