...
class ROA(object):
    ...
    Make sticker 
    '''
    def __init__(self, encoder, classifier, size):
        ...
        with torch.set_grad_enabled(False):
            ...
            for i in range(xtimes):
                for j in range(ytimes):
                    ...
                    all_loss = nn.CrossEntropyLoss(reduction='none')(classifier(encoder(sticker)),y)
    ...
    def gradient_based_search(self, X, y, alpha, num_iter, width, height, xskip, yskip, potential_nums,random = False):
        ...
        #print(X1.shape)
        #print(X1)
        loss = nn.CrossEntropyLoss()(classifier(encoder(X1)), y) 
        #print(loss)
        loss.backward()
        ...
        with torch.set_grad_enabled(False):
            for l in range(output_j1.size(1)):
                ...
                all_loss = nn.CrossEntropyLoss(reduction='none')(classifier(encoder(sticker1)),y)
